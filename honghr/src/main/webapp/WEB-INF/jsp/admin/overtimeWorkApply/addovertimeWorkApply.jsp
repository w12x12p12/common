...
<body>
		...
		<div class="col-sm-10">
			<c:forEach items="${codeList}" var="code">
				<label class="w_honghr_radio_label" title="${code.codeName}">
					<input type="radio" name="applyType" class="w_honghr_radio show_overtime_applyType" value="${code.codeValue}" />${code.codeName}
				</label>
			</c:forEach>
		</div>
	</div>
	<div class="form-group w_honghr_input w_edit_textArea clearfix">
		...
		<div class="col-sm-10">
			<textarea class="form-control w_honghr_textArea w_edit_applyReason" name="applyReason" placeholder="请输入加班原因"></textarea>
		</div>
	</div>
		<div class="form-group w_honghr_input">
		<label for="disabledSelect" class="col-sm-2 control-label"><b class="z_common_star">*</b>加班开始时间</label>
		<div class="col-sm-10">
			<input id="startDate" name="applyBeginTime" class="form-control w_date_input" type="text" placeholder="请输入开始时间" maxlength="100" onFocus="var endDate=$dp.$('endDate');WdatePicker({onpicked:function(){endDate.focus();},dateFmt:'yyyy-MM-dd H:00',isShowClear:false,maxDate:'#F{$dp.$D(\'endDate\',{d:0})}'})" onchange="changeDuration();" readonly="readonly"/>
		</div>
	</div>
	<div class="form-group w_honghr_input">
		<label for="disabledSelect" class="col-sm-2 control-label"><b class="z_common_star">*</b>加班结束时间</label>
		<div class="col-sm-10">
			<input id="endDate" name="applyEndTime" class="form-control w_date_input" type="text" placeholder="请输入结束时间" maxlength="100" onFocus="WdatePicker({minDate:'#F{$dp.$D(\'startDate\',{d:0})}',dateFmt:'yyyy-MM-dd H:00',isShowClear:false})" onchange="changeDuration();" readonly="readonly"/>
		</div>
	</div>
	<div class="form-group w_honghr_input">
	...
	<div class="form-group w_honghr_input">
		<label for="disabledSelect" class="col-sm-2 control-label"><b class="z_common_star">*</b>加班说明</label>
		<div class="col-sm-10 w_honghr_tips">
			<p>1.加班时间以2小时为基准，以1小时为进制，并以实际打卡时间为准。</p>
			<p>2.工作日加班，加班计时时间从19时后开始。</p>
			<p>3.周末及节假日加班以实际打卡时间为准，但应扣除相应的就餐时间（按每餐1小时计）。</p>
		</div>
	</div>
