...
public class BusinessTripApplyService {
	...
	public Pager<BusinessTripApplyVo> findForCheckPage(BusinessTripApplyVo search, Integer employeeId, int currentPage,
	...
	public Pager<BusinessTripApplyVo> findHasCheckPage(BusinessTripApplyVo search, Integer employeeId, int currentPage,
			int pageSize) throws UnsupportedEncodingException {
		List<Object> params = new ArrayList<Object>();
		params.add(employeeId);
	...
	public Integer getFirstCheckEmployeeId(Integer businessTripApplyId) {
		StringBuilder sb = new StringBuilder();
		sb.append("select check_employee_id ");
		sb.append("from business_trip_apply_check ");
		sb.append("where business_trip_apply_id = ? ");
		sb.append("and apply_check_sequence = ? ");
		return businessTripApplyDao.findCountBySql(sb.toString(), new Object[] { businessTripApplyId, 1 });
	}

	public Integer getNextCheckEmployeeId(Integer businessTripApplyId, Integer checkEmployeeId) {
		StringBuilder sb = new StringBuilder();
		sb.append("select bc.check_employee_id ");
		sb.append("from business_trip_apply_check bc ");
		sb.append("where bc.business_trip_apply_id = ? ");
		sb.append("and bc.apply_check_sequence = ( ");
		sb.append("select c.apply_check_sequence + 1 ");
		sb.append("from business_trip_apply_check c ");
		sb.append("where c.business_trip_apply_id = ? ");
		sb.append("and c.check_employee_id = ?) ");
		return businessTripApplyDao.findCountBySql(sb.toString(),
				new Object[] { businessTripApplyId, businessTripApplyId, checkEmployeeId });
	}
	...
	public void sendNoticeMail(Employee sender, Integer receiverId) {
		...
		if (receiver != null && StringUtils.isNotEmpty(receiver.getEmail())) {
			...
			receiverMail.setMail_subject(message.append(sender.getEmployeeName()).append("提交了新的出差申请单").toString());
			message.delete(0, message.length());
			receiverMail.setMail_content(
					message.append(sender.getEmployeeName()).append("在宏程教育人事考勤系统中提交了新的出差申请单，请您尽快审批。").toString());
			message.delete(0, message.length());
