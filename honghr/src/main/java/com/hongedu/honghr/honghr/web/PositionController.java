...
public class PositionController {
	...
	public JsonResult savePosition(Position position) {
		try {// 添加
			...
			if (positionParentId != null) {
				...
				positionInfo = positionService.findMaxByParentId(DataConstant.position_top_parentId);
			}
			...
			if (position.getPositionId() == null) {// 添加
				...
				if (position.getDepartmentId() == null) {// 不选部门默认为公司级
					Department department = departmentService.findByNUM(DataConstant.DEPARTMENT_TOP_NUM);
					position.setDepartmentId(department.getDepartmentId());
				}
				/* 判断positionParentId是否为空 */
				if (positionParentId == null) {// positionParentId为空
					position.setPositionParentId(DataConstant.position_top_parentId);
					// 查询以0为positionParentId的职务的最大PositionNum
					Position positiones = positionService.findMaxByParentId(DataConstant.position_top_parentId);
					/* 判断最大PositionNum是否为空 */
					if (positiones.getPositionNum() != null) {// 最大PositionNum不为空
						...
						position.setPositionNum(DataConstant.POSITION_TOP_NUM);
					}
					...
					if (positions.getPositionNum() != null) {
						...
						position.setPositionNum(POSNum + DataConstant.POSITION_SECOND_TOP_NUM);
					}
					positionInfo.setReferred(DataConstant.REFERRED);// 设置职位引用信息
	...
	public String positionList(
			...
			@RequestParam(required = false, defaultValue = DataConstant.pageSize) Integer pageSize, String positionName,
			Model model) {
		String positionNames = null;
		try {
			if (positionName != null) {
				positionNames = new String(positionName.replaceAll(" ", "").getBytes("ISO8859-1"), "UTF-8");
			}
			Pager<Position> page = positionService.findPage(positionNames, currentPage, pageSize);
			...
			model.addAttribute("positionName", positionNames);
