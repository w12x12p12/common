...
public class AbsenceApplyService {
	 ...
	 * 查询已申请(包括已通过)的休假时长
	 */
	public String getAbsenceApplyDurationForCheck(Integer employeeId, String applyType) {
		StringBuilder sb = new StringBuilder();
		sb.append("select sum(apply_duration) ");
		sb.append("from absence_apply ");
		sb.append("where employee_id = ? ");
		...
		String time = absenceApplyDao.getPropertyBySql(sb.toString(),
				new Object[] { employeeId, applyType, EnumApplyStatus.CHECK_LIMIT.getCode(), DataConstant.EXIST });
		return time != null ? time : "0";
	}
	...
	public Integer getNextCheckEmployeeId(Integer absenceApplyId, Integer checkEmployeeId) {
		StringBuilder sb = new StringBuilder();
		sb.append("select ac.check_employee_id ");
		sb.append("from absence_apply_check ac ");
		...
		sb.append("from absence_apply_check c ");
		sb.append("where c.absence_apply_id = ? ");
		sb.append("and c.check_employee_id = ?) ");
		return absenceApplyDao.findCountBySql(sb.toString(),
				new Object[] { absenceApplyId, absenceApplyId, checkEmployeeId });
	}
